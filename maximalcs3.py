...
class LabelClass(object):
    __slots__ = ["G_nodes", "X_G_nodes", "H_nodes", "is_adjacent"]
    ...
    def __repr__(self):
        return f"<{self.G_nodes}, {self.X_G_nodes}, {self.H_nodes}, {self.is_adjacent}>"
...
class PartitioningMCISFinder(object):
        ...
        for lc in label_classes:
            ...
            for u in lc.H_nodes:
                ...
                if edge_label not in label_to_new_lc:
                    is_adjacent = lc.is_adjacent or self.H.has_edge(w, u)
            ...
            for u in lc.X_G_nodes:
                edge_label = get_edge_label(self.G, self.edge_label_fun, v, u)
                if edge_label in label_to_new_lc:
                    label_to_new_lc[edge_label].X_G_nodes.append(u)
            for new_lc in label_to_new_lc.values():
                if new_lc.G_nodes or new_lc.X_G_nodes:
    ...
    def select_label_class(self, label_classes, assignment_count):
        if self.connected and assignment_count > 0:
            candidates = [lc for lc in label_classes if lc.is_adjacent and lc.G_nodes]
        else:
            candidates = [lc for lc in label_classes if lc.G_nodes]
        if not candidates:
            return None
    ...
    def search(self, label_classes, assignments):
        ...
        if label_class is None:
            if assignments:
                if self.connected:
                    is_maximal = not any(lc.X_G_nodes and lc.is_adjacent for lc in label_classes)
                else:
                    is_maximal = not any(lc.X_G_nodes for lc in label_classes)
                if is_maximal:
                    self.count += 1
                    print(self.count)
            return
        ...
        label_class.X_G_nodes.append(v)
        self.search(label_classes, assignments)
